...
class BEGAN(op):
    ...
    def generator(self, x, reuse=None):
        with tf.variable_scope('gen_') as scope:
            ...
            x = conv2d(x, [3, 3, f, 3], stride=1, name='conv5_a')
        return x
    ...
    def decoder(self, x, reuse=None):
        with tf.variable_scope('disc_') as scope:
            ...
            x = fc(x, 8 * 8 * f, name='dec_fc')
            x = tf.reshape(x, [-1, 8, 8, f])

            x = conv2d(x, [3, 3, f, f], stride=1, name='conv1_dec_a')
            x = tf.nn.elu(x)
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv1_dec_b')
            x = tf.nn.elu(x)

            x = tf.image.resize_nearest_neighbor(x, size=(w / 4, w / 4))
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv2_dec_a')
            x = tf.nn.elu(x)
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv2_dec_b')
            x = tf.nn.elu(x)

            x = tf.image.resize_nearest_neighbor(x, size=(w / 2, w / 2))
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv3_dec_a')
            x = tf.nn.elu(x)
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv3_dec_b')
            x = tf.nn.elu(x)

            x = tf.image.resize_nearest_neighbor(x, size=(w, w))
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv4_dec_a')
            x = tf.nn.elu(x)
            x = conv2d(x, [3, 3, f, f], stride=1, name='conv4_dec_b')
            x = tf.nn.elu(x)

            x = conv2d(x, [3, 3, f, 3], stride=1, name='conv5_dec_a')
        return x

    def build_model(self):

        # Input placeholder
        self.x = tf.placeholder(tf.float32, shape=[self.batch_size, 128], name='x')
